...
// Written by meego/fix-qml.py out of src/mimer.cpp: Qt 4.7 only allows
...
var Mimer = {
    ...
    TIFF: "image/tiff",
    SVG: "image/svg+xml",
    RBMP: "image/reverse-encoding-bmp",
    DOC: "application/msword",
    DOCX: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    RTF: "text/rtf",
    RTF_APP: "application/rtf",
    ODT: "application/vnd.oasis.opendocument.text",
    PPT: "application/vnd.ms-powerpoint",
    PPTX: "application/vnd.openxmlformats-officedocument.presentationml.presentation",
    ODP: "application/vnd.oasis.opendocument.presentation",
    Plaintext: "text/plain",
    OfficeFormats: ["application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/rtf", "application/rtf", "application/vnd.oasis.opendocument.text", "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation", "application/vnd.oasis.opendocument.presentation"],
    RasterFormats: ["image/pwg-raster", "image/urf"],
    isRaster: function(t) { return this.RasterFormats.indexOf(t) != -1 },
    isImage: function(t) { return t.indexOf("image/") == 0 && !this.isRaster(t) },
    isOffice: function(t) { return this.OfficeFormats.indexOf(t) != -1 }
}

// Harmattan has no calligraconverter and no way to install one.
var ConvertChecker = { calligra: false }

// strings.js, written in here: see above.
var media =
...
function supported_formats(printer, considerAdditionalFormats)
{
    ...
    if(has(formats, Mimer.PDF) ||
       (has(formats, Mimer.Postscript) || raster ))
    {
        pdf = true;
    ...
    if(pdf || has(formats, Mimer.Plaintext))
    {
        plaintext = true;
        mimetypes.push(Mimer.Plaintext);
    }

    if((ConvertChecker.calligra) && pdf)
    {
        office = true;
        mimetypes = mimetypes.concat(Mimer.OfficeFormats);
    }

    if(pdf || has(formats, Mimer.JPEG) || has(formats, Mimer.PNG) || has(formats, Mimer.RBMP))
    {
        images = true;
...
function supports_raster(printer)
{
    ...
    return (has(formats, Mimer.PWG) || has(formats, Mimer.URF));
}
...
function ippName(name, value, printerStrings)
{
    ...
    case "media-type":
        ...
        else if(printerStrings != undefined && printerhasOwnProperty("media-type."+value))
        {
...
var pdfTargets = [Mimer.OctetStream, Mimer.PDF, Mimer.Postscript, Mimer.PWG, Mimer.URF];
...
{
    var targets = [Mimer.OctetStream, Mimer.JPEG, Mimer.PNG, Mimer.RBMP,
                   Mimer.PWG, Mimer.URF, Mimer.PDF, Mimer.Postscript];
    return has(targets, type)
}
